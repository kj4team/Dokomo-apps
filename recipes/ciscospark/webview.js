...
module.exports = Dokomo => {
  ...
  const getMessages = () => {
    ...
    if (elements.length > 0 && elements[1].querySelector(UNREAD_BADGE_SELECTOR)) {
      directCount = Dokomo.safeParseInt(elements[1].querySelector(UNREAD_BADGE_SELECTOR).textContent);
    }
    if (elements.length > 1 && elements[2].querySelector(UNREAD_BADGE_SELECTOR)) {
      indirectCount = Dokomo.safeParseInt(elements[2].querySelector(UNREAD_BADGE_SELECTOR).textContent);
    }
